...
def get_closed_pores(bin_volume, structure_neighbors_num=6):
    """
    function returns a bin_volume of levitating pores for the
...
def plot_pore_volume_histogram(bin_volume,
    ...
    pore_volume_distribution = get_pore_volume_distribution(lv,
    ...
    stats = (f'total_num_of_pores = {total_num_of_pores}\n'
             f'MAX pore volume = {np.max(pore_volume_distribution):.0f}\n'
             f'MIN pore volume = {np.min(pore_volume_distribution):.0f}')
    bbox = dict(boxstyle='round', fc='blanchedalmond', ec='orange', alpha=0.35)
    ax.text(0.95, 0.3, stats, fontsize=9, bbox=bbox,
            transform=ax.transAxes, horizontalalignment='right')

    ax.hist(pore_volume_distribution,
            bins=bins,
            log=True,
            edgecolor='k')

    ax.set_title(f'pores_volume_distribution | connectivity number={structure_neighbors_num}')
    ax.set_xlabel('volume in voxels')
    ax.grid()

    if save_plot:
        save(figure, "plots", "pores_volume_distribution")
